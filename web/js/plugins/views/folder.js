...
//                                if ( json[i] ) {
                                    ...
                                    $div.find('.dropdown-menu')
                                        .prepend( $m.shared ? '' : '<li><a href="#" onClick="$m.view.folder.share(\''+p.replace(/'/g,"\\\'")+'\',\''+$m.state.servers[ $m.state.server ].url.replace(/api.php.*$/,'index.php').replace(/#.*$/,'')+'?link='+sharedToken+'\');" class="folder-share"><i class="icon-share"></i> Share</a></li>' )
